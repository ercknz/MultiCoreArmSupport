...
void SerialPackets::WritePackets(unsigned long &totalTime, RobotControl &Robot, unsigned long &lTime) {
  ...
  */ 
  ...
  for (int16_t i = _TX_goalQDOTcts_SLOT; i < _TX_goalXYZ_SLOT; i++){
    dataPacket[i] = packetBuffer[i - _TX_goalQDOTcts_SLOT];
  }
  // Robot Goal XYZ (meters)
  packetBuffer = floatArrayToBytes(Robot.GetGoalXYZ());
  for (int16_t i = _TX_goalXYZ_SLOT; i < _TX_BLANK_SLOT; i++){
